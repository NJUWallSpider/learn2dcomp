...
def evaluate_instances(input_dir=None, output_dir=None, model_path=None, problem=None):
    ...
    if input_dir is None:
        ...
        input_dir = config.PROCESSED_DATA_DIR / problem / folder_name
    else:
    ...
    # Dataset & Loader
    dataset = MILPDataset(files, transform=None)
    loader = DataLoader(
        dataset, 
        batch_size=1, 
        shuffle=False,
    ...
    with torch.no_grad():
        for i, data in enumerate(tqdm(loader)):
            ...
            # graph_voting_reassignment handles noise via constraint structure,
            # which produces structurally-sound decompositions.
            # reassign_noise_points can merge variables based solely on
            # embedding distance, causing degraded DW solution quality
            # (see round_004_gnn_oracle_mismatch.md). (Mirroring utilities.py)
...
if __name__ == "__main__":
    ...
    evaluate_instances(args.input, args.output, args.model, problem=args.problem)
